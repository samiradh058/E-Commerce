...
import { addAdmin } from "@/app/(content)/_utils/user";
...
export default function Add_Admin() {
  ...
  const loginSignup2Text = "Add New Admin";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      if (result && result.success) {
        ...
        alert(`Signup failed: ${result && result.error ? result.error : ""}`);
      }
    }
  };
  ...
  return (
    <>
      <button
        className="bg-primary text-white rounded-md hover:bg-blue-600 transition-all duration-200 h-fit py-1 px-2"
